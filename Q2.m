clear;
N=200;       % N is the times of the events
...
for i=1:N
    if rand<0.8     % 80% probablity of heads
...
text(0.03,N-sum(X)+4,num2str(N-sum(X)))
text(0.92,sum(X)+4,num2str(sum(X)))         % Show the numbers of H&Ts
